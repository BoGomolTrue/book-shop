<?php 
    ...
    $select_two="select * FROM reserve where  _UserID = '$u_id' and _BooksID= '$book_id'";
    ...
    if($num_two > 0){
        $update="update reserve set _AmountO = _AmountO - 1 where _BooksID='$book_id'";
        $update=mysqli_query($link,$update);
        echo "Книга успешно обновлена!";
        if($row_two['_AmountO'] <= 1){
            $delete = "delete from reserve where _UserID = '$u_id' and _BooksID = '$book_id'";
